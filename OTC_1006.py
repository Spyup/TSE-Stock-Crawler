...
class StockCrawling:
    ...
    def insert_sql(self, stock_dict):
        ...
        sql = "INSERT IGNORE INTO {}.otc_important_stock (catchtime, sdate, stime, sid, sname, price, thisamount, high, low, yesterday, open, amount) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)".format(database)
        records = []
    ...
    def search_sql(self,database,_dt,_t,stockID):
        ...
        sql_same_time = "SELECT * FROM {db}.otc_important_stock WHERE `sdate`='{sdate}' AND stime='{stime}' AND `sid`='{sid}'".format(db=database, sdate=_dt, stime=_t, sid=stockID)
...
if __name__ == '__main__':
    ...
    data = pd.read_csv("./OTC_Important_StockID.csv")
